...
class SearchTrackRenameChan extends Component {
    constructor(props) {
        super(props);
    ...
    createArea = () => {
        ...
        var objAction = {name: this.state.name};
        var objReaction = { idServer: this.state.serverReaction, idUser: this.state.user }
        var tmp = username + 'searchTracksRenameChannel' + Math.floor((Math.random() * 1000) + 1);
        ...
        var raw = JSON.stringify({
            ...
            "serviceReactionConfig": objReaction,
            "action": "searchTracks", "reaction": "renameChannel"
        });
        ...
        return (
            <div className="mid-dashboard">
                ...
                <img className="gmaile" src={Spotify} alt="gmail"/> 
                    ...
                    <h1 className="trigger">1. Search Track in Spotify</h1>
                    <h1 className="account">Enter your information</h1>
                    <input type="email" onChange={(text) => { this.handleName(text) }} autoFocus placeholder="Enter Track" name="email" className="input-spotify"></input>
...
export default SearchTrackRenameChan;