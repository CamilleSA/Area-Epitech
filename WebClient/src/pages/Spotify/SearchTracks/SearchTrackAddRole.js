...
class SearchTrackAddRole extends Component {
    constructor(props) {
        super(props);
    ...
    createArea = () => {
        ...
        var objAction = {name: this.state.name};
        var objReaction = { idServer: this.state.serverReaction, idUser: this.state.user, idRole: this.state.role }
        var tmp = username + 'searchTracksAddRole' + Math.floor((Math.random() * 1000) + 1);
        ...
        var raw = JSON.stringify({
            ...
            "serviceReactionConfig": objReaction,
            "action": "searchTracks", "reaction": "addRole"
        });
...
export default SearchTrackAddRole;