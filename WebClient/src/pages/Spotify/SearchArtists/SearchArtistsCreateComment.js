...
class SearchArtistsCreateComment extends Component {
    constructor(props) {
        super(props);
    ...
    createArea = () => {
        ...
        var objAction = {name: this.state.name};
        var objReaction = { token: tokenGithub, owner: this.state.user, repo: this.state.role }
        var tmp = username + 'searchArtistsDeleteRepo' + Math.floor((Math.random() * 1000) + 1);
        ...
        var raw = JSON.stringify({
            ...
            "serviceReactionConfig": objReaction,
            "action": "searchArtists", "reaction": "deleteRepo"
        });
        ...
        return (
            <div className="mid-dashboard">
                ...
                <img className="gmaile" src={Spotify} alt="gmail"/> 
                    ...
                    <input type="email" onChange={(text) => { this.handleName(text) }} autoFocus placeholder="Enter Artists" name="email" className="input-spotify"></input>
               </div>
            </div>
...
export default SearchArtistsCreateComment;