...
class SearchArtistsDelBranch extends Component {
    constructor(props) {
        super(props);
    ...
    createArea = () => {
        ...
        var objAction = {name: this.state.name};
        var objReaction = { token: tokenGithub, owner: this.state.user, repo: this.state.role, issue: this.state.channel }
        var tmp = username + 'searchArtistsCreateComment' + Math.floor((Math.random() * 1000) + 1);
        ...
        var raw = JSON.stringify({
            ...
            "serviceReactionConfig": objReaction,
            "action": "searchArtists", "reaction": "createComment"
        });
...
export default SearchArtistsDelBranch;