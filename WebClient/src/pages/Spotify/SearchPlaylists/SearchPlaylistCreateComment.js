...
class SearchPlaylistsCreateComment extends Component {
    constructor(props) {
        super(props);
    ...
    createArea = () => {
        ...
        var objAction = {name: this.state.name};
        var objReaction = { token: tokenGithub, owner: this.state.user, repo: this.state.role }
        var tmp = username + 'searchPlaylistsDeleteRepo' + Math.floor((Math.random() * 1000) + 1);
        ...
        var raw = JSON.stringify({
            ...
            "serviceReactionConfig": objReaction,
            "action": "searchPlaylists", "reaction": "deleteRepo"
        });
...
export default SearchPlaylistsCreateComment;