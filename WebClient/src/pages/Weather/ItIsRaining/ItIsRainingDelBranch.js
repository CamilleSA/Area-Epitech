...
class ItIsRainingDelBranch extends Component {
    constructor(props) {
        super(props);
        this.state = {
            city: '',
    ...
    createArea = () => {
        ...
        var objAction = { name: this.state.city };
        var objReaction = { token: tokenGithub, owner: this.state.user, repo: this.state.role, issue: this.state.channel }
        var tmp = username + 'BadWeatherCreateComment' + Math.floor((Math.random() * 1000) + 1);
        ...
        var raw = JSON.stringify({
            ...
            "action": "getBadWeather", "reaction": "createComment"
        });
...
export default ItIsRainingDelBranch;