...
class WeatherChangeCreateComment extends Component {
    constructor(props) {
        super(props);
        this.state = {
            city: '',
    ...
    createArea = () => {
        ...
        var objAction = { name: this.state.city };
        var objReaction = { token: tokenGithub, owner: this.state.user, repo: this.state.role }
        var tmp = username + 'WeatherDeleteRepo' + Math.floor((Math.random() * 1000) + 1);
        ...
        var raw = JSON.stringify({
            ...
            "action": "getWeather", "reaction": "deleteRepo"
        });
...
export default WeatherChangeCreateComment;