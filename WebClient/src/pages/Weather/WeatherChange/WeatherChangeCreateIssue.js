...
class WeatherChangeCreateIssue extends Component {
    ...
    createArea = () => {
        ...
        var objAction = { name: this.state.city };
        var objReaction = { token: tokenGithub, owner: this.state.user, repo: this.state.role }
        var tmp = username + 'WeatherCreateIssue' + Math.floor((Math.random() * 1000) + 1);
        ...
        var raw = JSON.stringify({
            ...
            "action": "getWeather", "reaction": "createIssue"
        });
...
export default WeatherChangeCreateIssue;