...
class WeatherChangeLinkPost extends Component {
    ...
    createArea = () => {
        ...
        var objAction = { name: this.state.city };
        var objReaction = { username: this.state.user, password: this.state.channel, subreddit: this.state.role }
        var tmp = username + 'WeatherLinkPost' + Math.floor((Math.random() * 1000) + 1);
        ...
        var raw = JSON.stringify({
            ...
            "action": "getWeather", "reaction": "postMessage"
        });
...
export default WeatherChangeLinkPost;