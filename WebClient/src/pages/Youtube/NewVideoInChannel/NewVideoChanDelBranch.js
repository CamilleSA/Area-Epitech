...
class NewVideoChanDelBranch extends Component {
    constructor(props) {
        super(props);
        this.state = {
            city: '',
    ...
    createArea = () => {
        ...
        var objAction = { channelName: this.state.email };
        var objReaction = { token: tokenGithub, owner: this.state.user, repo: this.state.role, issue: this.state.channel }
        var tmp = username + 'NewVideoChannelCreateComment' + Math.floor((Math.random() * 1000) + 1);
        ...
        var raw = JSON.stringify({
            ...
            "action": "newVideoInChannel", "reaction": "createComment"
        });
...
export default NewVideoChanDelBranch;