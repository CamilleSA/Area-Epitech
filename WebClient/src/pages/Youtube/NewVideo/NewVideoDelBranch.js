...
class NewVideoDelBranch extends Component {
    constructor(props) {
        super(props);
        this.state = {
            city: '',
    ...
    modifyArea = () => {
        ...
        myHeaders.append("Content-Type", "application/json; charset=UTF-8");
        var objAction = {};
        var objReaction = JSON.stringify({ token: tokenGithub, owner: this.state.user, repo: this.state.role, issue: this.state.channel })
    ...
    createArea = () => {
        ...
        var objAction = {};
        var objReaction = JSON.stringify({ token: tokenGithub, owner: this.state.user, repo: this.state.role, issue: this.state.channel })
        var tmp = username + 'NewVideoCreateComment' + Math.floor((Math.random() * 1000) + 1);
        ...
        var raw = JSON.stringify({
            ...
            "action": "newVideoInPopular", "reaction": "createComment"
        });
...
export default NewVideoDelBranch;