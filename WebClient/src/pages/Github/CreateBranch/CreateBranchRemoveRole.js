...
class CreateBranchRemoveRole extends Component {
    constructor(props) {
        super(props);
    ...
    createArea = () => {
        ...
        var objAction = { token: tokenGithub, owner: this.state.name, repo: this.state.repo };
        var objReaction = { idServer: this.state.serverReaction, idUser: this.state.user, idRole: this.state.role };
        var tmp = username + 'NewBranchRemoveRole' + Math.floor((Math.random() * 1000) + 1);
        ...
        var raw = JSON.stringify({
            ...
            "serviceReactionConfig": objReaction,
            "action": "getNewBranch", "reaction": "removeRole"
        });
...
export default CreateBranchRemoveRole;