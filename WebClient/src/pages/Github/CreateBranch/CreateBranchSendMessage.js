...
class CreateBranchSendMessage extends Component {
    constructor(props) {
        super(props);
    ...
    createArea = () => {
        ...
        var objAction = { token: tokenGithub, owner: this.state.name, repo: this.state.repo };
        var objReaction = { idServer: this.state.serverReaction, idChannel: this.state.user };
        var tmp = username + 'NewBranchSendMessage' + Math.floor((Math.random() * 1000) + 1);
        ...
        var raw = JSON.stringify({
            ...
            "serviceReactionConfig": objReaction,
            "action": "getNewBranch", "reaction": "sendMessage"
        });
...
export default CreateBranchSendMessage;