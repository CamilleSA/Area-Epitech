...
class CreateRepositoryLinkPost extends Component {
    constructor(props) {
        super(props);
    ...
    createArea = () => {
        ...
        var objAction = { token: tokenGithub, owner: this.state.serverAction };
        var objReaction = { username: this.state.user, password: this.state.channel, subreddit: this.state.role }
        var tmp = username + 'NewRepoLinkPost' + Math.floor((Math.random() * 1000) + 1);
        ...
        var raw = JSON.stringify({
            ...
            "serviceReactionConfig": objReaction,
            "action": "getNewRepo", "reaction": "postMessage"
        });
...
export default CreateRepositoryLinkPost;