...
class CreateCommitSendMessage extends Component {
    constructor(props) {
        super(props);
    ...
    createArea = () => {
        ...
        var objAction = { token: tokenGithub, owner: this.state.name, repo: this.state.repo };
        var objReaction = { idServer: this.state.serverReaction, idChannel: this.state.user };
        var tmp = username + 'NewCommitSendMessage' + Math.floor((Math.random() * 1000) + 1);
        ...
        var raw = JSON.stringify({
            ...
            "serviceReactionConfig": objReaction,
            "action": "getCommit", "reaction": "sendMessage"
        });
...
export default CreateCommitSendMessage;